...
3. Fedora
```
sudo dnf install nasm gcc-c++ binutils qemu-system-x86
```
4. MacOS
```
brew install nasm
brew install x86_64-elf-binutils   # gives you x86_64-elf-ld, x86_64-elf-objcopy
brew install x86_64-elf-gcc        # gives you a real cross g++ targeting ELF
brew install qemu
```
...
```
`x86_64-elf-g++ --version` should say something like `"g++ 13.x.x and"` and `x86_64-elf-ld --version` should mention `"ELF"`.
